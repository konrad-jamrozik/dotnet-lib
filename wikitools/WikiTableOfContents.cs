...
namespace Wikitools
{
    public record WikiTableOfContents : MarkdownDocument
    {
            ...
            Task<IEnumerable<WikiPageStats>> pagesStatsTask)
        {
            ...
            // kja 5 see Wikitools.AzureDevOps.WikiPageStatsPath.FromFileSystemPath
            var fsPathsTrie = new FilePathTrie(pagesPaths.Select(path => (string)WikiPageStatsPath.FromFileSystemPath(path)));
            var lines = fsPathsTrie.PreorderTraversal().ZipMatching(
                pagesStats,
