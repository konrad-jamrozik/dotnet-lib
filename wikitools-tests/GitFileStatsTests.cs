...
public class GitFileStatsTests
{
    ...
    [Fact] // kja currently failing test. See Wikitools.Lib.Git.GitLogFilePath.TryParseRename
    public void SumsByFilePathsWithRenamePresent()
    {
        ...
        var stats = GitFileStats.SumByFilePath(
            new[]
            {
                ...
                new GitLogCommit("Author1", now, new[]
                {
                    new GitLogCommit.Numstat(25, 15, "abc/{ => ghi/jkl}/def/foo.md"),
                }),
                // kja interleave in this tests also other rename chain
                new GitLogCommit("Author1", now, new[]
                {
                    new GitLogCommit.Numstat(6, 2, "abc/ghi/jkl/def/foo.md")
                }),
                // kja interleave rename loop
                new GitLogCommit("Author1", now, new[]
                {
                    new GitLogCommit.Numstat(10, 1, "abc/ghi/jkl/def/{foo.md => qux.md}")
                }),
                new GitLogCommit("Author1", now, new[]
                {
                    new GitLogCommit.Numstat(100, 400, "abc/ghi/jkl/def/qux.md")
                }),
            });
